...
public sealed class PostgresPersistenceTests
{
    ...
    public async Task Initial_migration_applies_and_enforces_core_constraints()
    {
        ...
        Assert.True(await ScalarBoolAsync(connection,
            "select to_regclass('knowledge.document_version') is not null", cancellationToken));
        Assert.True(await ScalarBoolAsync(connection,
    ...
    private static async Task<bool> ScalarBoolAsync(DbConnection connection, string sql, CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
        command.CommandText = sql;
